 ...
 *
 * File:    fragment_hdr_shaft.shader
 * Desc:    Render hdr shaft flare shader
 * Version: 1.02
...
void main() {
	...
	half3 shaft = sample.xyz * saturate(sample.w - threshold);
	
	for(int i = 1; i < 32; i++) {
		texcoord += direction;
		sample = texture2D(s_texture_0,texcoord);
		shaft += sample.xyz * (saturate(sample.w - threshold) * weight);
		weight *= 0.95f;
	}
	
	// 16.1258f = (1.0f - pow(0.95f,32.0f)) / (1.0f - 0.95f)
	gl_FragColor = half4(shaft * color.xyz * (2.0f / 16.1258f),1.0f);
}
...
half4 main(FRAGMENT_IN IN) : COLOR {
	...
	return half4(shaft * color.xyz * (2.0f / 16.1258f),1.0f);
}
...
float4 main(FRAGMENT_IN IN) : SV_TARGET {
	...
	return float4(shaft * color.xyz * (2.0f / 16.1258f),1.0f);
}
