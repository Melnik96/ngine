 ...
 *
 * File:    fragment_surface_light_omni.shader
 * Desc:    Water surface light omni shader
 * Version: 1.11
...
void main() {
	...
	#include <core/shaders/water/fragment_surface_foam.h>
		...
		#ifdef SHADOW
			color *= shadowOmni(s_texture_14,s_texture_15,gl_TexCoord[6],s_material_textures[14]);
		#endif
	#endif
	
	color *= textureCube(s_texture_13,gl_TexCoord[5].xyz).xyz;
...
half4 main(FRAGMENT_IN IN) : COLOR {
	...
	#ifdef QUALITY_MEDIUM
		#ifdef SHADOW
			color *= shadowOmni(s_texture_14,s_texture_15,IN.texcoord_6,s_material_textures[14]);
		#endif
	#endif
	
	color *= texCUBE(s_texture_13,IN.texcoord_5).xyz;
...
float4 main(FRAGMENT_IN IN) : SV_TARGET {
	...
	#ifdef QUALITY_MEDIUM
		#ifdef SHADOW
			color *= shadowOmni(s_texture_14,s_sampler_14,s_sampler_compare_14,s_texture_15,s_sampler_15,IN.texcoord_6);
		#endif
	#endif
	
	color *= s_texture_13.Sample(s_sampler_13,IN.texcoord_5).xyz;
...
half4 main(FRAGMENT_IN IN) : COLOR {
	...
	#ifdef QUALITY_MEDIUM
		#ifdef SHADOW
			color *= shadowOmni(s_texture_14,s_texture_15,IN.texcoord_6);
		#endif
	#endif
	
	color *= h3texCUBE(s_texture_13,IN.texcoord_5);
	return half4(color * s_light_color.xyz * (attenuation * fog),1.0f);
}
