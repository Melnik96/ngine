 ...
 *
 * File:    fragment_base_reflection_cube_ambient.shader
 * Desc:    Mesh base reflection cube ambient shader
 * Version: 1.32
...
void main() {
	...
	#ifdef USE_TESSELLATION && TESSELLATION
		world_normal = vertex_tangent * normal.x + vertex_binormal * normal.y + vertex_normal * normal.z;
		half3 camera_direction = normalize(normalize(s_camera_position - gl_TexCoord[3].xyz));
	#else
		world_normal.x = dot((gl_TexCoord[1].xyz),normal);
		world_normal.y = dot((gl_TexCoord[2].xyz),normal);
		world_normal.z = dot((gl_TexCoord[3].xyz),normal);
		half3 camera_direction = normalize(half3(gl_TexCoord[4].xyz));
	#endif
