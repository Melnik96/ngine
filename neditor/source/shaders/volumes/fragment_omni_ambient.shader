 ...
 *
 * File:    fragment_omni_ambient.shader
 * Desc:    VolumeOmni ambient shader
 * Version: 1.07
...
void main() {
	...
	half4 color = texture2D(s_texture_0,half2(length(gl_TexCoord[0].xyz),attenuation_transform.x));
...
half4 main(FRAGMENT_IN IN) : COLOR {
	...
	half4 color = tex2D(s_texture_0,half2(length(IN.texcoord_0),attenuation_transform.x));
...
float4 main(FRAGMENT_IN IN) : SV_TARGET {
	...
	half4 color = s_texture_0.Sample(s_sampler_0,half2(length(IN.texcoord_0),attenuation_transform.x));
...
half4 main(FRAGMENT_IN IN) : COLOR {
	...
	half4 color = h4tex2D(s_texture_0,half2(length(IN.texcoord_0),attenuation_transform.x));
