 ...
 *
 * File:    vertex_occlusion_light.shader
...
void main() {
	...
	gl_TexCoord[1].xy = s_attribute_1.xy * s_viewport.xy / 4.0f;
	
	gl_TexCoord[2].x = s_attribute_0.x * projection.x;
...
VERTEX_OUT main(VERTEX_IN IN) {
	...
	OUT.texcoord_1 = IN.texcoord.xy * s_viewport.xy / 4.0f;
	
	OUT.texcoord_2.x = IN.position.x * projection.x;
