...
void main() {
	...
	#include <core/shaders/particles/fragment_base_volume.h>
		...
		#ifdef SHADOW
			color *= shadowProj(s_texture_14,gl_TexCoord[5],s_material_textures[14]);
		#endif
	#endif
	
	attenuation *= half(half(gl_TexCoord[4].z) > 0.0f);
	color.xyz *= texture2DProj(s_texture_13,gl_TexCoord[4]).xyz;
...
half4 main(FRAGMENT_IN IN) : COLOR {
	...
	#ifdef QUALITY_MEDIUM
		#ifdef SHADOW
			color *= shadowProj(s_texture_14,IN.texcoord_5,s_material_textures[14]);
		#endif
	#endif
	
	attenuation *= (IN.texcoord_4.z > 0.0f);
	color.xyz *= tex2Dproj(s_texture_13,IN.texcoord_4).xyz;
...
float4 main(FRAGMENT_IN IN) : SV_TARGET {
	...
	#ifdef QUALITY_MEDIUM
		#ifdef SHADOW
			color *= shadowProj(s_texture_14,s_sampler_compare_14,IN.texcoord_5);
		#endif
	#endif
	
	attenuation *= (IN.texcoord_4.z > 0.0f);
	color.xyz *= s_texture_13.Sample(s_sampler_13,IN.texcoord_4.xy / IN.texcoord_4.w).xyz;
	return color * attenuation;
}
...
half4 main(FRAGMENT_IN IN) : COLOR {
	...
	#ifdef QUALITY_MEDIUM
		#ifdef SHADOW
			color *= shadowProj(s_texture_14,IN.texcoord_5);
		#endif
	#endif
	
	attenuation *= (IN.texcoord_4.z > 0.0f);
	color.xyz *= h3tex2Dproj(s_texture_13,IN.texcoord_4.xyw);
	return color * attenuation;
}
